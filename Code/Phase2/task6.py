...
def calculate_distance_with_all_images(query_img_matrix, wt_matrix, types):
    dist_dict = {}
    for i in range(len(wt_matrix)):
        data_vector = wt_matrix[i]
        dist_dict[types[i]]=np.abs(np.linalg.norm(data_vector - query_img_matrix))
...
def distance_to_similarity(dist_dict):
    similarity_dict = {}
    for type in dist_dict:
        similarity_dict[type] = '{0:.8f}'.format((1/dist_dict[type]) * 100)
    return similarity_dict


def display_type(dist_dict, similarity_dict, type_of_query_image):
    ...
    print("\nType Label Associated: "+type_of_query_image+"\n")
...
if __name__ == "__main__":
    ...
    type_of_query_image, dist_dict = calculate_distance_with_all_images(img_matrix_in_latent_features, wt_mat, types)
    similarity_dict = distance_to_similarity(dist_dict)
    display_type(dist_dict,similarity_dict,type_of_query_image)