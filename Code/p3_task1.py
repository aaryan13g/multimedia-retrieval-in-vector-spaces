...
class DTree:
    #implements a decision tree classifier algorithm.
    ...
    def _best_split(self, X, y):
        ...
        for f_index in range(n_cols):
            X_curr = X[:, f_index]
            # For every unique value of that feature
            for threshold in np.unique(X_curr):
                df = np.concatenate((X, y.reshape(1, -1).T), axis=1)
                df_left = np.array([row for row in df if row[f_index] <= threshold])
                df_right = np.array([row for row in df if row[f_index] > threshold])
